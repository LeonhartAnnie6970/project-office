...
export function AdminReport() {
  ...
  const handleExport = async (format: "pdf" | "excel") => {
    try {
      ...
      if (!token) {
        alert("Anda belum login. Silakan login sebagai admin terlebih dahulu.")
        return
      }
      ...
      if (!response.ok) {
        // coba baca body JSON bila ada, tampilkan pesan error
        const ct = response.headers.get("content-type") || ""
        let msg = `Failed to generate report (status ${response.status})`
      ...
      link.download = `laporan-tiket-${selectedStatus}-${Date.now()}.${ext}`
